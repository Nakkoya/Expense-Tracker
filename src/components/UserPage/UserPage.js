...
const Paginate = ({ page, totalPage }) => {
	return (
		<Pagination
			...
			variant='outlined'
			color='primary'
			renderItem={(item) => <PaginationItem {...item} component={Link} to={`/userpage?page=${item.page}`} />}
		/>
	);
};
					...
					<Box>
						<Card sx={style.card}>
							<CardMedia
								component='img'
								sx={style.cardMedia}
								src={useravatarFull !== "" ? `${useravatarFull}` : defaultAvatar}
								alt='avatar'
							/>
						...
						>
							...
							{expenseList?.length > 0 && (
								<>
									<UserPageContext.Provider
										value={{ setChangelogID: setChangelogID, setOpenChangelog: setOpenChangelog }}
									>
										<TableContainer
											component={Paper}
											sx={{
											...
											<Table aria-label='expense'>
												...
												<TableBody>
													{expenseList && expenseList.length > 0 ? (
														expenseList.map((item) => <ExpenseList key={item._id} data={item} />)
													) : (
														<CircularProgress />
													)}
												</TableBody>
							...
							{expenseList?.length === 0 && <h1>You don't have any record yet</h1>}
						</Box>
					</Box>
					<Modal open={openChangelog} onClose={() => setOpenChangelog(false)}>
						<Box sx={style.modal}>{openChangelog && <Changelog data={changelog} func={setOpenChangelog} />}</Box>
					</Modal>
				</Container>
			)}
		</>
